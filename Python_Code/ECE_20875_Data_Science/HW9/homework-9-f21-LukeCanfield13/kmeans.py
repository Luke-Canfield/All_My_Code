...
def kmeans(point_data, cluster_data):
    ...
    point_ls = makePointList(point_data)
    # 2. Make list of clusters using createClusters and cluster_data
    cluster_ls = createClusters(cluster_data)
    ...
    while call:
      ...
      for point in point_ls:
        call |= point.moveToCluster(point.closest(cluster_ls))
      for clusters in cluster_ls:
       clusters.updateCenter()
    clusters = cluster_ls  
