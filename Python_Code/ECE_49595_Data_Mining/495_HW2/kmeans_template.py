...
def assignCluster(dataSet, k, centroids):
    ...
    clusterAssment = []
    for data in dataSet:
        dist = [np.linalg.norm(data - centroid) for centroid in centroids]
        closest_cluster = np.argmin(dist)
        clusterAssment.append(closest_cluster)
...
def getCentroid(dataSet, k, clusterAssment):
    ...
    for cluster_ID in range(k):
        cluster_point = [dataSet[i] for i, cluster in enumerate(clusterAssment) if cluster == cluster_ID]
...
if __name__ == '__main__':
    ...
    save_filename = data_filename.replace('.csv', '_with_cluster.csv')
