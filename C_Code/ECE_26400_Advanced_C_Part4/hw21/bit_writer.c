...
void write_bits(BitWriter* a_writer, uint8_t bits, uint8_t num_bits_to_write) {
	...
	if(num_bits_to_write > 0) {
		if(num_bits_to_write > a_writer -> num_bits_left) {
			int num_bits_excess = num_bits_to_write - a_writer -> num_bits_left;
			num_bits_to_write = num_bits_excess;
			write_bits(a_writer, bits >> num_bits_excess, a_writer -> num_bits_left);
		}
		uint8_t shift_var = bits << (8 - num_bits_to_write);
		a_writer -> current_byte |= shift_var >> (8 - a_writer -> num_bits_left); 
