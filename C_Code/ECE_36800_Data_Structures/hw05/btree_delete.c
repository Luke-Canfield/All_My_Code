...
void merge_subtree_with_next(BTreeNode* parent, int subtree_1_idx) {
	...
	int subtree_2_init_num_keys = subtree_2 -> num_keys;
	int subtree_2_key_idx = 0;
	(subtree_1 -> num_keys)++;
	subtree_1 -> keys[(subtree_1 -> num_keys) - 1] = parent -> keys[subtree_1_idx];


	for(int i = subtree_1 -> num_keys; i < MAX_KEYS; i++) {	
		(subtree_1 -> num_keys)++;
		subtree_1 -> keys[(subtree_1 -> num_keys) - 1] = subtree_2 -> keys[subtree_2_key_idx];
		subtree_2_key_idx++;
		(subtree_2 -> num_keys)--;
	}
	if(subtree_2_init_num_keys >= MAX_KEYS - 2) {
		parent -> keys[subtree_1_idx] = subtree_2 -> keys[MAX_KEYS - 2];
		subtree_2 -> keys[0] = subtree_2 -> keys[MAX_KEYS - 1];
		(subtree_2 -> num_keys)--;
	}
		...
		if(left_child -> num_keys >= MIN_KEYS + 1) {
			...
			if(right_child -> num_keys >= MIN_KEYS + 1) {
				...
				if(root -> num_keys == MAX_KEYS) {
					...
					merge_subtree_with_next(root, key_idx - 1);
				}
				else {
					merge_subtree_with_next(root, key_idx);
				}
		...
		if(new_root -> num_keys == MIN_KEYS) {
			...
			if(key_idx == 0) {
				if(right_child -> num_keys >= MIN_KEYS + 1) {
					...
					if(root -> num_keys == MAX_KEYS) {
						key_idx = MAX_KEYS - 1;
						for(int i = key_idx; i >= 0; i--) {
							merge_subtree_with_next(root, i);
						}
					}
				}
			}
